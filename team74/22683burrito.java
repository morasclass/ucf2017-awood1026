...
public class burrito {
	public static void main(String[] args){
		...
		for(int i=0; i<cases; i++){
			...
			boolean shi = false;
			if((new BigDecimal("" + r).multiply(new BigDecimal("2")).add(new BigDecimal(bl)).compareTo(new BigDecimal("" + w)) < 1)){
				if(c.compareTo(new BigDecimal("" + l)) < 1){
					shi = true;
				}
			}
			else if((new BigDecimal("" + r).multiply(new BigDecimal("2")).add(new BigDecimal(bl))).compareTo(new BigDecimal("" + l)) < 1) {
